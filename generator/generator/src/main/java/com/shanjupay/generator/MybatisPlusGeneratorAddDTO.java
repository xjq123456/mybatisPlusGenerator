...
public class MybatisPlusGeneratorAddDTO {
        ...
        public static void getDto() throws JsonProcessingException {
            ...
            globalConfig.setEntityName("%sAddDTO");
            strategyConfig.setEntityLombokModel(true);//实体类使用lombok
            ...
            // 要移除的字段
            List<String> fieldsToRemoveAdd = Arrays.asList("is_delete", "status", "id", "create_time", "update_time", "create_user_id", "update_user_id");;
            ...
            if(conditionStr.contains("AddDTO")){
                filterPredicate = item -> !fieldsToRemoveAdd.contains(item);
            }
            ...
            List<TableField> tl = tableList.get(0).getFields().stream().filter(field->!fieldsToRemoveAdd.contains(field.getName())).collect(Collectors.toList());
